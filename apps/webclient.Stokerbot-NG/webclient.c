...
{
	printf("DNS resolved %s to %u.%u.%u.%u\r\n", name, uip_ipaddr1(ipaddr),
	       uip_ipaddr2(ipaddr), uip_ipaddr3(ipaddr), uip_ipaddr4(ipaddr));
}
...
void webclient_connect(void)
{
	if (tickDiffS(timer_webclient) > 140) {
		printf("No reply to webclient recieved for 2minutes (%u -- %u -- %u), resetting...",
		       tickDiffS(timer_webclient), timer_webclient, tickS);
	...
	if (uip_connected() || uip_rexmit()) {
		...
		for (uint8_t i = 0; i < MAXSENSORS; i++) {
			if (sensorValues[(i * SENSORSIZE) + FAMILY] != 0) {
				int frac = sensorValues[(i * SENSORSIZE)
				                        + VALUE2] *
