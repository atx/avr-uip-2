...
void enc28j60Init(uint8_t *macaddr)
{
	...
	CSPASSIVE; // ss=0
	//
	ENC28J60_CONTROL_DDR  |= 1 << ENC28J60_CONTROL_SI |
	                         1 << ENC28J60_CONTROL_SCK; // mosi, sck output
	...
	*/        //
	...
	// note this chip doesn't do full duplex well... so, don't try.
...
uint16_t enc28j60PacketReceive(uint16_t maxlen, uint8_t *packet)
{
	...
	if (gNextPacketPtr - 1 >
	    RXSTOP_INIT) { // RXSTART_INIT is zero, no test for gNextPacketPtr less than RXSTART_INIT.
